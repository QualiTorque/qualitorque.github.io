...
3. Define the grain's details. These are the mandatory settings:
    ...
    * Remove the __host__ section, and specify the execution host as an input of ```type: execution-host```.
6. If the grain should run after the deployment of another grain, add ```__depends_on: <name of grain dependency>``` to the new grain:
  ```jsx title=
grains:
  new-blueprint-grain:
    kind: blueprint
