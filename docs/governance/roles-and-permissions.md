...
Now that you understand how <ProductName /> works, it's time to understand how groups, roles and permissions work, and invite other members of your team to your <ProductName /> account and spaces. 

**In this article:**

- [Groups in <ProductName />](#groups-in-torque)
- [Roles in <ProductName />](#roles-in-torque)
- [Invite Users to <ProductName />](#invite-users-to-torque)
...
## Groups in <ProductName />

Groups are an efficient way to manage multiple users together.
A <ProductName /> group consists of:
- The group permissions
- The group users

A user can be a member of multiple groups. <ProductName /> will calculate the super-set of all the permissions that the user has in the platform (those that were given them explicitly and those which they inherited from the set of groups they belong to). 
...
If your <ProductName /> account is configured to use SSO for authentication, the assignment of SSO users to groups in <ProductName /> can be synchronized automatically from the IdP assignments (currently supported in Okta and Azure active directory).

For detailed instructions on how to map <ProductName /> user groups to your IdP groups, click [here](/admin-guide/sso#mapping-torque-user-groups-to-idp-groups).

## Roles in <ProductName />

As Account Admin, you can invite users to your <ProductName /> account and add them to spaces, while Space Admins can only add existing <ProductName /> users to the spaces they administer. 
...
As such, there are 4 roles in <ProductName />:
* **Account Admin** has full permissions in all <ProductName /> spaces, and can access the Administration area, which is not accessible to space admins. The account admin is responsible for setting up <ProductName /> for the first time, general supervision and ongoing maintenance. This includes account-level responsibilities, like inviting users to the <ProductName /> account, creating spaces, and adding cloud account and Kubernetes compute services, general supervision and ongoing maintenance, and must be able to operate as a space administrator.
...
| [Manage agents](/torque-agent/Install-and-connect-self-hosted-agent)	   |Install new <ProductName /> agents (requires K8s cluster or Vcenter access), delete agents, edit agents properties.| ![Locale Dropdown](/img/green-dot.png)      |||||
| [Manage cloud accounts for cost collection](/governance/cost-tracking/cost)	   |Add/remove cloud accounts for <ProductName /> to collect environment cost from.| ![Locale Dropdown](/img/green-dot.png)      |||||
...
| [Manage users](#invite-users-to-torque)	   |Invite users to <ProductName />, cancel users invitations, add users to space, change users roles.| ![Locale Dropdown](/img/green-dot.png)        ||||
...
| [Manage space notifications](/admin-guide/notifications)	   |Create/delete/modify target for <ProductName /> notifications for space events.| ![Locale Dropdown](/img/green-dot.png)       |![Locale Dropdown](/img/green-dot.png)||||
...
| [Manage space repositories](/getting-started/asset-discovery)	  |1. Add/remove git repositories containing <ProductName /> blueprints or IaC assets to a space.<br />2. Run discovery on the repositories to find and add assets to <ProductName /> (as auto-generated blueprints). | ![Locale Dropdown](/img/green-dot.png)        |![Locale Dropdown](/img/green-dot.png)|||
| [Manage space users and roles](#add-existing-users-to-your-space)	   |Add <ProductName /> users to a space and set their role in the space.| ![Locale Dropdown](/img/green-dot.png)        |![Locale Dropdown](/img/green-dot.png)|||
| [Manage space blueprints](/blueprint-designer-guide/blueprint-quickstart-guide)	   |1. Modify the contents of a blueprint that resides in <ProductName /> (not in source control) or delete it.<br />2. Edit blueprint metadata (labels, icons).| ![Locale Dropdown](/img/green-dot.png)        |![Locale Dropdown](/img/green-dot.png)  |![Locale Dropdown](/img/green-dot.png)||
...
| View agents	   |View the list of <ProductName /> agents connected to an account.| ![Locale Dropdown](/img/green-dot.png)        |![Locale Dropdown](/img/green-dot.png)|![Locale Dropdown](/img/green-dot.png)|||
...
| [Run API calls](/rest-api)   |Use <ProductName />'s REST API to perform different operations (based on permissions). | ![Locale Dropdown](/img/green-dot.png)        |![Locale Dropdown](/img/green-dot.png)  |![Locale Dropdown](/img/green-dot.png)|  ![Locale Dropdown](/img/green-dot.png)|
...
6. Click **Send Invitation**.
  The users will get a "Welcome to <ProductName />" invitation email, prompting them to login to the space. Once they log in, they will be granted the space role and permissions you specified.

## Add Existing Users to Your Space

<ProductName /> role: Account admin, space admin

1. In the suitable <ProductName /> space, select **Settings** from the left menu.
2. Click the **Users** tab.
3. Click the **Add Users** button.
4. Select the <ProductName /> user you wish to add to this space. Repeat to add additional users.
5. Click **Done**.
...
In some situations, the account admin may want to add users to the <ProductName /> account without sending invitations to the users. 
