...
<ProductName /> supports connecting Quali CloudShell as an asset repository. CloudShell blueprints are treated as assets in <ProductName /> and can be referenced using the 'CloudShell' grain type.

## Prerequisites

1. CloudShell user with at least "External Extended" permissions in the CloudShell Domains that you want to use.
2. A self-hosted <ProductName /> agent that can reach the Sandbox Api of your CloudShell instance.
3. CloudShell version 2021.2 and higher.

## Configuration

1. Register CloudShell as a Repository Provider via <ProductName /> REST API. If the API call is successful it will return a GUID representing the ID of the new repository provider. Save this ID for the next step.
...
```
  ...
  * Replace __&lt;TOKEN&gt;__ with a valid <ProductName /> token with <ProductName /> Admin permissions
  * Replace __&lt;PROVIDER_NAME&gt;__ with a display name for the CloudShell instance. Example: "CloudShell_Dev"
  * Replace __&lt;CLOUDSHELL_SERVER_URL&gt;__ with the IP or DNS of your CloudShell instance. The CloudShell instance needs to be reachable from at least one of your self-hosted <ProductName /> agents. <ProductName /> will automatically select an agent for communicating with CloudShell
...
```
  ...
  * Replace __&lt;SPACE_NAME&gt;__ with the relevant <ProductName /> space 
  * Replace __&lt;TOKEN&gt;__ with a valid <ProductName /> token with <ProductName /> Admin permissions
  ...
  * Replace __&lt;SANDBOX_API_PORT>&gt;__ with the port of the CloudShell Sandbox API. The default port is 82
  * Replace __&lt;REPOSITORY_NAME&gt;__ with the name you would like to assign to this repository. This name will be used in <ProductName /> Blueprints to reference this repo
