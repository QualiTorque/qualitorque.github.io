...
- Kuberentes cluster - can be any cluster, including on your on-premise network. Please note that <ProductName /> __does not support__ cluster nodes on ARM architecture.
- [Outbound Ports for Kubernetes Cluster Nodes](/torque-agent/torque-outbound-ports) must be open to allow <ProductName /> to access and communicate with the cluster.
...
- One or more target namespaces on the cluster where the <ProductName /> agent will create resources.
...
- Authentication and permissions - The agent will need sufficient permissions to create the environment's cloud resources. There are a couple of ways to provide the permissions, depending on where the environment resources will be created. 
  ...
  - To create resources on your cloud using Terraform:
      ...
      - If your cluster is an __AKS__ (resources will be created on Azure): Provide the account's authentication credentials when creating the agent in <ProductName />. For details, see [Terraform Authentication on AKS](/torque-agent/service-accounts-for-azure).
      ...
      - For other types of clusters, or if you want to connect to your AWS/Azure with your basic credentials, there is no built-in authentication with <ProductName /> so there are no pre-requisites related to authentication and permissions. You can store your cloud credentials in the <ProductName /> secret store and use them for your TF deployment.

## Setup

1. In <ProductName />'s **Administration** page, open the **Cloud Accounts** tab.
...
5. Paste the command in your command-line window to deploy the agent to your cluster. For example:     
    ...
    ```
5. A __Connected!__ status is displayed in <ProductName />, indicating that the agent was successfully installed and can communicate with <ProductName />. 
...
If the agent fails to connect with <ProductName />, you can try the following to identify the problem.
...
1. Make sure the agent pod is running and healthy. You can run the following command on your cluster: 
     ...
     ```
2. Make sure outbound http connection to <ProductName /> is open:  
     ```bash
